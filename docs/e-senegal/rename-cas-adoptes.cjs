 ...
 */
async function nextSequenceNumber(prefix, padding) {
  // prefix = 'PINS-METIER-' ou 'PINS-TECH-'
  // padding = 3 ou 4
  const regex = `${prefix}(\\d+)`;
  const result = await prisma.$queryRawUnsafe(`
    SELECT COALESCE(MAX(CAST(substring(code from '${regex}') AS INTEGER)), 0) AS max_num
    FROM cas_usage_mvp
    WHERE code LIKE '${prefix}%'
  `);
  const nextNum = parseInt(result[0].max_num) + 1;
...
(async () => {
  ...
  for (const cas of candidats) {
    ...
    if (codeOriginal && (codeOriginal.startsWith('PINS-METIER-') || codeOriginal.startsWith('PINS-TECH-'))) {
      ...
      nouveauCode = await nextSequenceNumber(prefix, padding);
    }
    ...
    if (INTERACTIVE && APPLY) {
      ...
      for (const cas of ambigus) {
        ...
        if (rep.toLowerCase().startsWith('m')) {
          const newCode = await nextSequenceNumber('PINS-METIER-', 3);
          ...
          const newCode = await nextSequenceNumber('PINS-TECH-', 4);
