...
export function PartenaireCasDetailPage() {
  ...
  const relTech = cas.relationsMetier || []; // services techniques mobilisés depuis un cas métier
  const relMet = cas.relationsTechnique || []; // parcours métier servis par un cas technique
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" className="bg-teal hover:bg-teal-dark" onClick={() => setShowManifestDialog(true)}>
            Manifester un intérêt
          </Button>
        </div>
      </div>

      {/* Avertissement manifestation */}
      {showManifestDialog && (
        <Card className="border-amber-300 bg-amber-50">
          <CardContent className="pt-4 space-y-3">
            <p className="font-semibold text-amber-900">Fonctionnalité disponible début juin 2026</p>
            <p className="text-sm text-amber-800">
              Le dépôt formel d'une manifestation d'intérêt sur ce cas d'usage sera disponible
              avec la livraison du module PTF complet en juin 2026.
            </p>
            <p className="text-sm text-amber-800">
              En attendant, vous pouvez exprimer votre intérêt par email auprès de la Delivery Unit MCTN :
              <a href="mailto:du@mctn.gouv.sn" className="ml-1 text-teal-700 hover:underline">
                <Mail className="inline w-3.5 h-3.5 mr-1" />du@mctn.gouv.sn
              </a>
            </p>
            <div className="flex justify-end">
              <Button variant="outline" size="sm" onClick={() => setShowManifestDialog(false)}>
                Fermer
              </Button>
            </div>
          </CardContent>
