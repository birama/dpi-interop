...
export function StakeholdersTable({ casUsageId, stakeholders, onChanged }: Props) {
  ...
  const autoSaisisAvecMotif = actifs.filter((s: any) => s.autoSaisine && s.motifAutoSaisine);
  ...
  return (
    <>
      <div className="bg-white rounded-lg border shadow-sm">
        ...
        {/* Zone expansible : motifs d'auto-saisine */}
        {autoSaisisAvecMotif.length > 0 && (
          <details className="border-t border-gray-100 group">
            <summary className="p-3 bg-gray-50 cursor-pointer hover:bg-gray-100 flex items-center gap-2 text-xs font-semibold text-navy list-none">
              <ChevronDown className="w-3.5 h-3.5 transition-transform group-open:rotate-180" />
              Voir les motifs d'auto-saisine ({autoSaisisAvecMotif.length})
            </summary>
            <div className="divide-y divide-gray-100">
              {autoSaisisAvecMotif.map((sh: any) => (
                <div key={sh.id} className="px-4 py-2.5 text-xs">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-semibold text-navy">{sh.institution?.code}</span>
                    <span className="text-[10px] text-teal bg-teal/10 px-1.5 py-0.5 rounded border border-teal/30 font-semibold">
                      Auto-saisie
                    </span>
                    {sh.typeConcernement && (
                      <span className="text-[10px] text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded">
                        {TYPE_CONCERNEMENT_LABELS[sh.typeConcernement]}
                      </span>
                    )}
                    {sh.dateAjout && (
                      <span className="text-[10px] text-gray-400 ml-auto">
                        {new Date(sh.dateAjout).toLocaleDateString('fr-FR')}
                      </span>
                    )}
                  </div>
                  <p className="text-gray-700 whitespace-pre-wrap">{sh.motifAutoSaisine}</p>
                </div>
              ))}
            </div>
          </details>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-[520px] max-w-[92vw]">
        ...
        <div className="p-5 space-y-3">
          <div className="p-3 bg-amber-50 rounded-lg border border-amber-200 flex gap-2 text-xs text-amber-800">
            ...
            <div>
              Votre institution ne sera plus partie prenante de ce cas d'usage.
              Vous perdrez l'acces aux details techniques, au fil d'avis et a l'historique.
              Cette action est tracee et l'initiateur recevra une notification.
            </div>
          </div>
          <div>
      ...
      <div className="bg-white rounded-xl shadow-2xl w-[560px] max-w-[92vw]">
        ...
        <div className="p-5 space-y-3">
          <div className="p-3 bg-red-50 rounded-lg border border-red-200 flex gap-2 text-xs text-red-800">
            <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
            <div>
              Cette action est <b>definitive</b> et sera notifiee a l'institution concernee avec le motif saisi.
              L'institution evincee ne pourra pas se reinscrire sans votre validation explicite.
